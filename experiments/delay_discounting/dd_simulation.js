...
// choiceProbLL. Adding a new model requires no changes here.
 ...
 * @property {Function} choiceProbLL - (design, params) => P(response = 1 = LL).
 ...
 *
 * The response is drawn from model.choiceProbLL — the exact likelihood used by the
...
function simulateDelayDiscountingChoice(design, simulation_config, rng, model) {
  ...
  const p_ll = model.choiceProbLL(design, params);
  const draw = rng();
