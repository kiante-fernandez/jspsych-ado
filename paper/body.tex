...
\texttt{jspsych-ado} brings \emph{adaptive design optimization} (ADO) to browser-based behavioral experiments built with jsPsych \citep{deleeuw2015jspsych}. After each response, a Bayesian model written in Stan \citep{carpenter2017stan} --- compiled to WebAssembly and run client-side in a Web Worker --- updates the posterior over the model parameters, and the next stimulus is chosen to maximize the expected information
