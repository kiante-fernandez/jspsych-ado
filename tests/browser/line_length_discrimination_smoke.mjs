...
try {
  ...
  for (const spec of specs) {
    ...
    if (!r.errored) {
      ...
      note(
        ["A", "B", "C"].includes(r.choiceLabel),
        `${mode}: choice label is A/B/C (got ${r.choiceLabel})`,
      );
      note(
      ...
      note(
        typeof r.simPA === "number" &&
          typeof r.simPB === "number" &&
          typeof r.simPC === "number" &&
          typeof r.simDraw === "number",
