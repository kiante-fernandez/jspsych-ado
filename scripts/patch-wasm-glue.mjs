...
export function patchSource(source) {
  ...
  if (source.includes(UNPATCHED))
    return { changed: true, source: source.split(UNPATCHED).join(PATCHED) };
...
export async function listModelMains() {
  ...
  for (const base of [MODELS_DIR, DEMOS_DIR]) {
    ...
    for (const entry of entries) {
      ...
      try {
        await access(file);
        mains.push({ name: entry.name, dir, file });
      } catch {
        /* no compiled wasm here */
      }
    }
  }
  return mains;
...
async function main() {
  ...
  for (const { name, file } of await listModelMains()) {
    ...
    try {
      ...
      console.log(`  WARNING: ${name}/main.js could not be read: ${e.message}`);
      missing.push(name);
      continue;
    }
    ...
    if (changed) {
      ...
      console.log(`  patched ${name}/main.js`);
      patched++;
    } else if (unrecognized) {
      console.log(
        `  WARNING: ${name}/main.js has neither the patched nor the known unpatched form`,
      );
      missing.push(name);
    } else {
      console.log(`  already patched ${name}/main.js`);
      already++;
    }
  }
  console.log(
